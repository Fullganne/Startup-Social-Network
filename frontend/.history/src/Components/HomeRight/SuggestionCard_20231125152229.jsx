...
const SuggestionCard = ({ user, key, followings }) => {
    ...
    useEffect(() => {         // Check if the user is in the list of followings\
        ...
        const isFollowed = followings.some(             (item) => item.followed === user.id && item.users === userData.id         );         // Update the state based on whether the user is followed         setFollowed(isFollowed);         console.log("CHECKINGGGGGG: " + followed);     }, []);
